...
class MyGUIConan(ConanFile):
    ...
    def validate(self):
        if self.settings.os != "Macos":
            raise ConanInvalidConfiguration(
                "The pinned MyGUI migration recipe currently supports macOS only"
            )
        deployment_target = self.settings.get_safe("os.version")
        if (
            deployment_target is None
            or Version(str(deployment_target)) < Version(MACOS_DEPLOYMENT_TARGET)
        ):
            raise ConanInvalidConfiguration(
    ...
    def generate(self):
        ...
        for source_prefix, mapped_prefix in prefix_maps:
            ...
            # macOS exposes /tmp as a symlink to /private/tmp. Conan reports
            # the resolved path, while CMake may emit the lexical /tmp path in
            # __FILE__. Map both spellings to the same stable prefix.
            lexical_tmp = "/tmp"
            ...
            for prefix_alias in prefix_aliases:
                ...
                if unsafe_characters:
                    raise ConanInvalidConfiguration(
                        "MyGUI compiler prefix-map path contains unsupported "
                        f"characters: {unsafe_characters!r}"
                    )
