...
part, `rad`, `deg`, integer-exponent `pow`, exact-significand `fmod`,
subnormal-aware integer-exponent `ldexp`, `clamp`, bounded variadic `min`/`max`,
exact `pi`/`huge` constants, typed variables, and flattened scalar `$components`
paths. Missing
...
`case` signatures and eighteen-name deterministic numeric allowlist,
