...
ValidationResult ValidateGeometry(const Ogre14ProceduralRoadCapture &capture,
  ...
  if (require_lineage &&
      (capture.stable_graphics_id == 0U ||
       capture.topology_revision == 0U)) {
    return Failure(ValidationCode::INVALID_IDENTIFIER, "road.lineage",
  ...
  if (index_count % 3U != 0U) {
    return Failure(ValidationCode::SIZE_MISMATCH, "road.indices",
  ...
  for (std::size_t index = 0U; index < index_count; ++index) {
    if (capture.indices[index] > 65535U ||
      ...
      return Failure(ValidationCode::VALUE_OUT_OF_RANGE, "road.indices",
                     "promoted uint16 index is outside the captured vertex range",
                     index);
    }
  }
  ...
  for (std::size_t index = 0U; index < index_count; index += 3U) {
    ...
    const Float3 face = Cross(Subtract(capture.positions[i1],
                                       capture.positions[i0]),
                              Subtract(capture.positions[i2],
                                       capture.positions[i0]));
...
ValidationResult ValidateConfiguration(
  ...
  if (configuration.maximum_live_roads >
          configuration.maximum_lifetime_roads ||
  ...
  if (state.lifecycle_ !=
    ...
    return Failure(
        ...
        state.lifecycle_ == Ogre14ProceduralRoadIdentityLifecycle::TOMBSTONED
            ...
            : "procedural object already has a manager-owned graphics identity");
  }
...
ValidationResult
Ogre14ProceduralRoadInventory::RegisterDerivedIdentityForAudit(
...
ValidationResult DeriveOgre14ProceduralRoadObjectId(
    std::uint64_t manager_graphics_id, std::uint64_t &derived_object_id) {
  ...
  for (const char byte : std::string_view(
           kProceduralRoadObjectIdentityDomain,
  ...
  for (std::uint32_t shift = 0U; shift < 64U; shift += 8U) {
    HashByte(candidate,
             static_cast<std::uint8_t>((manager_graphics_id >> shift) &
                                       0xFFU));
  }
  if (candidate == 0U) {
    return Failure(ValidationCode::INVALID_IDENTIFIER,
                   "road.derived_object_id",
...
ValidationResult BuildOgre14ProceduralRoadGeometryStateKey(
  ...
  const std::size_t index_bytes = capture.indices.size() * sizeof(std::uint32_t);
  candidate.reserve(sizeof(kProceduralRoadGeometryStateDomain) +
                    vertex_bytes + index_bytes + 48U);
  ...
  AppendU64(candidate,
            static_cast<std::uint64_t>(capture.positions.size()));
...
ValidationResult ResolveOgre14ProceduralRoadCacheEntry(
  ...
  if (previous != nullptr) {
    if (previous->topology_revision == 0U ||
  ...
  validation = BuildOgre14ProceduralRoadMeshPayload(capture, mesh_payload);
  if (!validation) {
    return validation;
  }
...
ValidationResult Ogre14ProceduralRoadInventoryTransaction::Build(
  ...
  for (std::size_t ordered_index = 0U; ordered_index < ordered.size();
    ...
    if (!HasInvertibleAffineTransform(capture.render_from_object) ||
      ...
      return Failure(LinearDeterminant(capture.render_from_object) < 0.0F
    ...
    if (capture.positions.size() >
      ...
      return Failure(ValidationCode::VALUE_OUT_OF_RANGE,
                     "road_inventory.geometry_count",
                     "road geometry exceeds configured per-road bounds",
                     ordered_index);
    }
    ...
    validation = DeriveOgre14ProceduralRoadObjectId(
    ...
    const auto previous = inventory.cache_by_manager_id_.find(
        capture.stable_graphics_id);
    Ogre14ProceduralRoadCacheEntry cache_entry;
    validation = ResolveOgre14ProceduralRoadCacheEntry(
        capture,
        previous != inventory.cache_by_manager_id_.end()
            ? &previous->second
            : nullptr,
        cache_entry);
    if (!validation) {
    ...
    section.exact_entity_name =
        CanonicalRoadName(capture.stable_graphics_id);
    ...
    section.mesh_identity.reverse_winding =
        capture.material.cull ==
        Ogre14GraphicsSceneMaterialCull::ANTICLOCKWISE;
    ...
    current_cache.emplace(capture.stable_graphics_id,
                          std::move(cache_entry));
    candidate_inventory.known_manager_ids_.insert(
        capture.stable_graphics_id);
  }
