...
void RoR::RegisterGameScript(asIScriptEngine *engine)
{
    ...
    result = engine->RegisterObjectMethod("GameScriptClass", "BeamClassPtr @spawnTruckDeterministic(string &in, vector3 &in, vector3 &in, uint64, uint64)", asMETHOD(GameScript, spawnTruckDeterministic), asCALL_THISCALL); ROR_ASSERT(result >= 0);
