...
bool TObjParser::ParseObjectLine(TObjEntry& object)
{
    ...
    // m_line_number is the parser's legacy zero-based counter (and is kept
    // that way in autogenerated instance names for compatibility). Expose
    // the authored, one-based line number for exact source-file policies.
    object.source_line = static_cast<std::size_t>(m_line_number) + 1U;
