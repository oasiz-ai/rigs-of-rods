...
HDR-specific lock records exact SHA-256 values for `HdrUtils.cpp`, the
...
and the `R10G10B10A2_UNORM` bloom intermediates.
...
- compare an analytic equation value to its binary32 shader-equation value
  using `abs(a-b) <= 2e-6 + 2e-5 * max(abs(a), abs(b))`;
